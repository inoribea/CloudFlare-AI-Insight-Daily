...
export async function handleWriteData(request, env) {
    ...
    try {
        ...
        // 如果请求中没有提供 foloCookie，或者显式要求使用 KV，尝试从 KV 中获取
        if ((!foloCookie || foloCookie === 'kv') && env.DATA_KV && env.FOLO_COOKIE_KV_KEY) {
            foloCookie = await env.DATA_KV.get(env.FOLO_COOKIE_KV_KEY);
            console.log(`Using foloCookie from KV: ${!!foloCookie}`);
        }

        if (foloCookie) {
            console.log(`Starting /writeData process with cookie: ${foloCookie.substring(0, 15)}...`);
        }
